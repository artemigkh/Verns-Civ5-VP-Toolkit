...
Sources (generated by ``db_util/db_export.py``):
  ...
  * ``db_util/out/leader_attributes.csv`` — civ × leader-attribute values
  * ``db_util/out/leader_info.csv``    — civ → leader name + victory pursuits
"""
...
# ===========================================================================
# Leaders report metadata
# ===========================================================================

@dataclass
class LeaderAttributeGroup:
    """One heatmap table: a named group and the attributes it spans, in order."""
...
@dataclass
class LeaderAttributes:
    # Group display order, each with its own ordered attribute list.
    groups: list[LeaderAttributeGroup]
    # civ -> group -> attribute -> value
    values: dict[str, dict[str, dict[str, int]]]
    # civ -> {leader, primary, secondary}; primary/secondary are "" when unset.
...
def load_leader_attributes() -> LeaderAttributes:
    """The four leader-attribute groups plus per-civ leader names/pursuits.

    ``leader_attributes.csv`` is already ordered by (group_order,
    attribute_order, civ) — the game's own attribute ordering — so groups and
    attributes are read out in file order rather than re-sorted here.
    """
    df = pd.read_csv(LEADER_ATTRIBUTES_CSV)

    groups: list[LeaderAttributeGroup] = []
    for name, group in df.groupby("group", sort=False):
        attributes: list[str] = []
        seen: set[str] = set()
        ...
        groups.append(LeaderAttributeGroup(name=name, attributes=attributes))

    values: dict[str, dict[str, dict[str, int]]] = {}
    for row in df.itertuples(index=False):
        values.setdefault(row.civ, {}).setdefault(row.group, {})[row.attribute] = int(
            row.value
        )

    info = pd.read_csv(LEADER_INFO_CSV).fillna("")
    leaders = {
        row.civ: {
            "leader": row.leader,
            "primary": row.primary_victory_pursuit,
            "secondary": row.secondary_victory_pursuit,
        }
        for row in info.itertuples(index=False)
    }

    return LeaderAttributes(groups=groups, values=values, leaders=leaders)
