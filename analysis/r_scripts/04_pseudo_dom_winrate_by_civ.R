...
# Drop victory types absent from the data so the legend only shows the ones
# present (e.g. Time vanishes when no game ended on the turn-limit).
present_vtypes_aug <- winrate_long %>%
...
winrate_long <- winrate_long %>%
    filter(!is.na(victory_type)) %>%
    mutate(victory_type = factor(as.character(victory_type),
                                 levels = intersect(VICTORY_LEVELS_AUG,
                                                    present_vtypes_aug)))
...
save_plot_dark(
    build_winrate(vtc_lut_b, IPSUM_VP_DARK_BG, theme_report_dark,
                  ...
                  caption_color = "grey60",
                  with_caption = FALSE),
    "04b_pseudo_dom_winrate_by_civ_nocap", width = 14, height = 7)
