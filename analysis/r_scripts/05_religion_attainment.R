# 05: Religion attainment-time KDEs (pantheon / founded / enhanced / reformed).
...
build_attainment <- function(theme_fn, bg) {
    ggplot(attainment_df, aes(x = turn, color = type)) +
        ...
        theme(legend.position = c(0.98, 0.98),
              ...
              plot.caption = element_text(size = 11, hjust = 1, face = "italic"))
}

save_plot(build_attainment(theme_report, IPSUM_VP_BG),
          "05a_religion_attainment_times", width = 12, height = 6)
save_plot_dark(build_attainment(theme_report_dark, IPSUM_VP_DARK_BG),
               "05b_religion_attainment_times", width = 12, height = 6)
save_plot_dark_nocap(build_attainment(theme_report_dark, IPSUM_VP_DARK_BG),
                     "05b_religion_attainment_times_nocap",
                     width = 12, height = 6)
