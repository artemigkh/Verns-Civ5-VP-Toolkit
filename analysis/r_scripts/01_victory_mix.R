# 01: Victory mix -- donut + violin+beeswarm.
# Produces:
#   01a_donut_violin_bees.png   light theme + vtc_lut_a (warm-gray bg)
#   01b_donut_violin_bees.png   dark theme  + vtc_lut_b (near-black bg)
...
victory_counts <- game_result_df %>%
    ...
    mutate(
        ...
        label = sprintf("%s\n%d  (%s)", victory_type, games, percent(share, 1))
    )
...
build_donut <- function(lut, bg, label_color) {
    ggplot() +
        geom_polygon(data = slice_polys,
                     aes(x = x, y = y, group = victory_type, fill = victory_type),
        ...
        geom_text(data = leader_df,
                  ...
                  vjust = 0.5, color = label_color,
                  show.legend = FALSE) +
        coord_fixed(xlim = c(-1.85, 1.85), ylim = c(-1.25, 1.25), clip = "off") +
...
# --- Variant a: light theme + vtc_lut_a -------------------------------------
plot_a <- compose(
    build_donut(vtc_lut_a, IPSUM_VP_BG, "grey25"),
    build_violin_bees(vtc_lut_a, IPSUM_VP_BG, theme_report, "black"),
    theme_report, "grey25"
)
save_plot(plot_a, "01a_donut_violin_bees", width = 14, height = 7)

# --- Variant b: dark theme + vtc_lut_b --------------------------------------
plot_b <- compose(
